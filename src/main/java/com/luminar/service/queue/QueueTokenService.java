...
public interface QueueTokenService {
	...
	/**
	 * Book one active new token for a customer for a given service. 
	 * (WAITING / SERVING) per customer per service.
	 */
	...
	/**
	 * Get recent past tokens (COMPLETED / SKIPPED) for customer. Limit controls how
	 * many records to return.
	 */
	...
	/**
	 * Get current queue status for a service (current serving + waiting count).
	 */
	ServiceQueueStatusDTO getServiceQueueStatus(Long serviceId);

	/**
	 * Call next token for the service assigned to staff. Moves token WAITING →
	 * SERVING.
	 */
